...
# Create dataframes of hyperparameters for the grid search
...
# Initialise model control - different, as it uses grid search
model_control = trainControl(
  method = "adaptive_cv", # 'cv' for cross validation, 'adaptive_cv' drops unpromising models
  number = 5, # number of folds in cross validation (or number of resampling iterations)
  repeats = 5, # number of repeats for repeated cross validation
  ...
  timing = system.time( 
    for(i in 1:number_of_folds)
    {
      ...
      if(runWoE)
      {
        ...
        prediction_ANN_woe = predict(ANN_woe, newdata = validation_fold_woe, type = "raw")
        ...
        if(perform_normalization)
        {
          ...
          prediction_ANN_norm = predict(ANN_norm, newdata = validation_fold_woe, type = "raw")
